...
*   [automatic installation](#automatic): quick, no fuss, no scripting required, and yet entirely customizable, including complete API access if the need arises
*   [manual installation](#manual-install): install on demand, individual customization both in HTML and JSON syntax
*   [pure JavaScript installation](#javascript-install): complete and consistent control via JavaScript, no dependence on external libraries, advanced configuration and seamless API integration
...
*   [purely JavaScript based](#javascript-install) - precondition: a JavaScript [clip](#clip-and-sources) object in the [player configuration](#player-options)
...
*   Fine-tuning the configuration on the [clip](#clip-options) and [source](#source-options) level is not possible or only in a restricted way for [playlists](playlists.html).
...
*   players can be individually customized even with this automated installation: [all options on all levels](#configuration-summary) are also available in [HTML notation](#configuration).
...
Advantages of the manual method over [automatic installation](#automatic):

*   you control where and **when** to install the player - not confined to page load
*   you can specify a [player specific configuration in JavaScript](#local-configuration) as argument to the `flowplayer` jQuery extension.

Advantage of the manual method over [pure JavaScript installation](#javascript-install):
...
This installation method uses `flowplayer()` as pure JavaScript function. It takes a reference to the container element as first argument, and the [player configuration](#player-options) as mandatory second argument.
...
Advantages of the pure JavaScript installation in addition to those of the [manual installation](#manual-install):

*   It does not depend on an external library - you may still use jQuery of course.
*   Availability of some advanced features with [playlists](playlist.html), like [subtitles](subtitles.html) or [looping of individual clips](#clip-options).
...
*   The page will load faster because no VIDEO tag is present, especially with multiple [splash setups](#splash).

Restrictions of the pure JavaScript installation:

*   Only one player can be installed per `flowplayer()` call.
*   By definition options set via [HTML data attributes](#html-configuration) have no effect.
...
The `clip` [configuration object](#clip-options) is the representation of a HTML5 VIDEO tag, including [Flash video](#flash-video), in [JavaScript Object Notation](http://www.secretgeek.net/json_3mins.asp).

Each object in a clip's `sources` [array](#source-options) is the representation of a HTML5 SOURCE tag in [JavaScript Object Notation](http://www.secretgeek.net/json_3mins.asp).
...
The Flowplayer [engines](#engines) can play a slew of [video formats](#video-formats): all videos which can be played by a [HTML5 video](#html-video) tag or by [Flash](#flash-video).
...
The audience will watch **one** clip - the content -, but to grant an optimal viewing experience the player should be provided with **several** sources to [choose from](#picking-order):
...
Then simply switch the player to live mode by setting the `live` option for [player](#player-options) or [clip](#clip-options) to `true`.
...
By default all HLS levels are shown in the HD menu. Manual selection can be disabled or a specific set of HLS levels specified with the `hlsQualities` [option](#player-options) on the [global](#global-configuration), [player](#player-options), and [clip](#clip-options) level.
...
The following format combinations ensure best cross browser and cross device compatibility with Flowplayer. The formats are listed in the recommended [order](#picking-order). You can also see which combinations are on offer at our [Flowplayer Drive](drive.html#result-files) transcoding and delivery platform.
...
You can change the ratio in the [global player configuration](#global-configuration):
...
Or you can set the ratio in the [HTML configuration](#html-configuration) using the `data-aspect-ratio` or `data-ratio` attribute of the container element as follows:
...
You may also let the player size dynamically be determined by the video's aspect ratio: Set the `adaptiveRatio` [player option](#player-options) to `true`.

This comes in handy for quick setups when you do not happen to know the exact dimensions of the video, but still want the player's screen size to match the aspect ratio of the video. By consequence refrain from configuring `adaptiveRatio` for a [splash setup](#splash) as the video's dimensions - and therefore its aspect ratio are only available once the video is loaded and the player is [ready](/docs/api.html#properties).

## Start screen

In a basic setup like the one in our [quick start guide](quickstart.html#flowplayer) the video is loaded and the first frame is shown - unless `autoplay` is [configured](#player-options).
...
*   [poster](#poster): the video is loaded in the background
*   [splash](#splash): the video is loaded on demand, i.e. when the user starts playback with a click
...
*   the `poster` [configuration option](#player-options) is set or
*   the VIDEO tag carries a `poster` attribute and
*   the [splash](#splash) setup is not enforced.
...
Or you configure the addition of the "is-splash" state class by setting the `splash` [option](#player-options) to `true` or to the location of the splash image. For instance this [global configuration](#global-configuration) applies the splash setup to all players on the page:
...
The following tables show all options as they are declared in a [global](#global-configuration), [local](#local-configuration) or [pure](#javascript-install) JavaScript configuration in JSON syntax.
...
The following options are `undefined` by default, but are supported by player extensions or additionally loaded plugins. Follow the link in the third column for details. The rightmost column indicates whether the option can also be set as data attribute in a [HTML configuration](#html-configuration). Some of the options accept a nested configuration object as value, in which case only the top-level option is listed here.
...
*   the Flash swf file: `flowplayer.swf` - if not loaded from our CDN (the default) the `swf` [option](#commercial-configuration) or the `data-swf` [container attribute](#html-configuration) must be set
*   the Flash HLS swf file: `flowplayerhls.swf` - if not loaded from our CDN (the default) the `swfHls` [option](#commercial-configuration) or the `data-swf-hls` [container attribute](#html-configuration) must be set
...
*   [manual installation (jQuery)](#manual-install): 1st argument (optional)
*   [pure JavaScript installation](#javascript-install): 2nd argument - mandatory, as a `clip` must be configured

Here is an example how to configure the `ratio` [option](#player-options) for all players which are manually installed into containers of class "player".
...
Local configuration overrides [global configuration](#global-configuration).
...
The HTML syntax allows to customize specific players even when they are [automatically installed](#automatic).

HTML configuration overrides [global configuration](#global-configuration) and [local JavaScript configuration](#local-configuration).
...
2.  [local JavaScript configuration](#local-configuration)
3.  [global JavaScript configuration](#global-configuration)
...
If you are in control of the IFRAME's source and the page where the IFRAME is shown you can allow fullscreen by excplicitly setting the `fullscreen` [configuration option](#player-options) to `true` on the originating page and specifying the `allowfullscreen` attribute for the IFRAME:
...
To allow fullscreen playback from within an IFRAME on iOS, additionally the `native_fullscreen` [option](#player-options) must be set to `true`.
...
The primary engine is the `html5` engine, unless you configured a different engine for a specific [source](#source-options) explicitly.
...
3.  the `engine` [source option](#source-options) for this source is not set to a value other than "flash" and
4.  the Flash plugin is enabled in the browser

or if

1.  the `engine` [source option](#source-options) for this source is set to "flash" and
2.  the Flash plugin is enabled in the browser

The Flowplayer Flash component requires Flash version 9.0.0\. Flash supports playback of the [MP4 format](#video-formats) since version 9.0.115\. Consider it safe to neglect offering a FLV source as fallback for ancient Flash.

The Flash engine supports playback of the following [video formats](#video-formats):
...
Flowplayer supports [HLS](#video-formats) playback with its [Flash engine](#flash-engine). Adaptive Bit Rate (ABR) switching is available not only in browsers which can play HLS in HTML5 video, but also in all other browsers, noteably most desktop browsers.
...
*   The `rtmp` [option](#player-options) for the server address - also called 'net connection url' - _must_ be set. Please consult the documentation of your server for its exact value.
...
The same applies to loading the video [sources](#source-options):
...
For finer control of [Flash HLS](#flash-hls) the following options are available as properties of the `flashls` configuration object on the [clip level](#clip-options):
...
Both forms of the `rtmp` option can be given at [player](#player-options) and [clip](#clip-options) level.
...
Installation of a `playlist` of sources into an [empty container element](/v5docs/setup.html#empty-container) has grown up to become a first class citizen in the form of the API friendly [JavaScript installation method](#javascript-install) in Flowplayer 6 with an extensible [clip object](#clip-object). We recommend to embrace the more flexible full syntax which will give you even more control over each [clip](#clip-options) and its [sources](#source-options).

Flowplayer 6 introduces a simpler source [picking order](#picking-order): Every source will be tried in the given order by all engines available. In the usual recommended setups this will not cause any difference in behaviour.

In the same vein, the `engine` option has moved from the [player level](/v5docs/setup.html#config-options) to the [source level](#source-options) to allow for complete fine-grained control of video source preferences.

Users of advanced RTMP options like `bufferTime`, `rtmpt`: The options will currently still work when set at player level directly. We recommend to start upgrading to the new [rtmp configuration object](#rtmp-options) which is available both at [player](#player-options) and [clip](#clip-options) level and thereby offers increased flexibility.